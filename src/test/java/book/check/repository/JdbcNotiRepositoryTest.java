...
//		noti.setN_no(1L);
	...
	@Test
	void 공지수정() {
		Noti noti = new Noti("제목2", "내용3", "어때책첵");
		Noti saveNoti = repository.saveNoti(noti); //아래에서 수정테스트를 위해 수정 전 데이터 저장
		
		Noti updateNoti = new Noti("수정함", "내용4", "나눔책첵");
		repository.updateNoti(saveNoti.getN_no(), updateNoti);
		
		assertThat(repository.findByNo(saveNoti.getN_no()).get().getN_title()).isEqualTo(updateNoti.getN_title());
	}
}
